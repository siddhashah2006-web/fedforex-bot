// Liq Grab v12 — CONTINUATION optimizer with HONEST out-of-sample selection.
// Build candidates -> split time 50/50 -> grid-search on TRAIN -> crown champion by TEST win rate.
const IST=5.5*3600;
...
function candidates(name,B){
  ...
  for(let k=42;k<B.length;k++){const b=B[k];const kz=kzOf(b.min);if(!kz)continue;const A=atr(B,k);if(A<=0)continue;
    ...
    if(!dir)continue;const want=dir==="long"?"bull":"bear",poi=dir==="long"?"sup":"res";
    const d1=trendAt(Ds,b.t),h4=trendAt(H4s,b.t),h1=trendAt(H1s,b.t);const pdPos=(b.c-drLo)/rng;
...
(async()=>{
  let CANDS=[];for(const name of Object.keys(SYM)){let B=null;try{B=await getBars(name);}catch(e){}if(!B||B.length<2000){console.log("skip "+name);continue;}CANDS=CANDS.concat(candidates(name,B));}
  const ts=CANDS.map(c=>c.t);const mid=(Math.max(...ts)+Math.min(...ts))/2;
  console.log(`Candidates ${CANDS.length}, ~${((Math.max(...ts)-Math.min(...ts))/(365.25*86400)).toFixed(2)}y, split at midpoint\n`);
  const groups=new Map();for(let i=0;i<CANDS.length;i++){const c=CANDS[i];const key=c.name+"|"+c.day;(groups.get(key)||groups.set(key,[]).get(key)).push(i);}
  for(const arr of groups.values())arr.sort((a,b)=>CANDS[a].t-CANDS[b].t);
  const GRID={trend:["all3","h4h1"],stack:[2,3,4],kz:["London","NY-AM","Lon+AM","all"],pd:["none","half","deep"],wick:[0,0.5,0.6],rrI:[1,2,3],mgmt:[1]};
  function pass(c,g){
    if(!(g.trend==="all3"?(c.d1===c.want&&c.h4===c.want&&c.h1===c.want):(c.h4===c.want&&c.h1===c.want)))return false;
    if(c.stack<g.stack)return false;
    if(g.kz==="Lon+AM"){if(c.kz!=="London"&&c.kz!=="NY-AM")return false;}else if(g.kz!=="all"&&c.kz!==g.kz)return false;
    ...
    if(c.wick<g.wick)return false;return true;}
  function ev(g,lo,hi){let n=0,w=0,l=0,tot=0;for(const arr of groups.values()){let ch=-1;for(const idx of arr){const c=CANDS[idx];if(c.t<lo||c.t>=hi)continue;if(pass(c,g)){ch=idx;break;}}if(ch<0)continue;const R=CANDS[ch].out[g.rrI][g.mgmt];n++;tot+=R;if(R>0.05)w++;else if(R<=-0.5)l++;}return{n,wr:(w+l)?w/(w+l)*100:0,exp:n?tot/n:0,tot};}
  const lbl=g=>`${g.trend} stk>=${g.stack} ${g.kz} pd:${g.pd} wick>=${g.wick} RR${RRS[g.rrI]} tiered`;
  // grid on TRAIN
  const cfgs=[];
  for(const trend of GRID.trend)for(const stack of GRID.stack)for(const kz of GRID.kz)for(const pd of GRID.pd)for(const wick of GRID.wick)for(const rrI of GRID.rrI)for(const mgmt of GRID.mgmt){
    const g={trend,stack,kz,pd,wick,rrI,mgmt};const tr=ev(g,-1e18,mid);if(tr.n>=30&&tr.exp>0)cfgs.push({g,tr});}
  // rank by TRAIN win rate, take top 40, evaluate on TEST
  cfgs.sort((a,b)=>b.tr.wr-a.tr.wr);
  const top=cfgs.slice(0,40).map(c=>({...c,te:ev(c.g,mid,1e18)}));
  const robust=top.filter(c=>c.te.n>=25).sort((a,b)=>b.te.wr-a.te.wr);
  console.log("===== CHAMPIONS by OUT-OF-SAMPLE (TEST yr2) win rate =====");
  console.log("TESTwin TESTexp  TRAINwin  n(tr/te)   CONFIG");
  for(const c of robust.slice(0,12))console.log((c.te.wr.toFixed(1)+"%").padStart(6),(c.te.exp.toFixed(2)+"R").padStart(7),(c.tr.wr.toFixed(1)+"%").padStart(8),`  ${c.tr.n}/${c.te.n}`.padEnd(10),lbl(c.g));
  // full-period stats of the robust champion
  if(robust.length){const champ=robust[0];const full=ev(champ.g,-1e18,1e18);
    console.log("\n===== ROBUST CHAMPION (best TEST win rate) =====");console.log(lbl(champ.g));
    console.log(`  TRAIN: ${champ.tr.n} trades, ${champ.tr.wr.toFixed(1)}% win, ${champ.tr.exp.toFixed(3)}R`);
    console.log(`  TEST : ${champ.te.n} trades, ${champ.te.wr.toFixed(1)}% win, ${champ.te.exp.toFixed(3)}R`);
    console.log(`  FULL : ${full.n} trades, ${full.wr.toFixed(1)}% win, ${full.exp.toFixed(3)}R, ${full.tot.toFixed(0)}R total`);}
})();
